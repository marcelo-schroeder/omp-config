...
async function runHookCommand(hook, { session, originalCwd, mode }) {
	const env = buildWorktreeChildEnv({
		...
		originalCwd,
		extraEnv: {
			OMPW_HOOK_EVENT: hook.event,
			OMPW_HOOK_MODE: mode,
			OMPW_HOOK_NAME: hook.name,
			OMPW_HOOK_SOURCE_KIND: hook.sourceKind,
			OMPW_HOOK_SOURCE_PATH: hook.sourcePath,
		},
	});
...
export async function runConfiguredHooks({ config, event, mode, session, originalCwd, debug = false }) {
	...
	for (const hook of hooks) {
		...
		if (debug) {
			console.error(`[ompw] hook source`, hook.sourcePath);
			console.error(`[ompw] hook command`, hook.command);
		}
		...
		if (hook.onFailure === "continue") {
			console.warn(`ompw: ${message} Continuing because onFailure=continue.`);
			continue;
		}
