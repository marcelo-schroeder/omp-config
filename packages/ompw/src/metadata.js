...
const METADATA_FILENAME = "ompw.json";
...
export function getOmpwMetadataFilename() {
	return METADATA_FILENAME;
}

export async function getManagedWorktreeMetadataPath(worktreePath) {
	const result = await runGit(["rev-parse", "--git-path", METADATA_FILENAME], worktreePath, "Failed to determine the ompw metadata path.");
...
export async function readManagedWorktreeMetadata(worktreePath) {
	...
	try {
		...
		if (error instanceof SyntaxError) {
			throw new Error(`Failed to parse ompw metadata at '${metadataPath}'.`);
		}

		throw new Error(`Failed to read ompw metadata at '${metadataPath}'.`);
	}
}
