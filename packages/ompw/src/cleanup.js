...
function canPrompt() {
	return Boolean(process.stdin.isTTY) || process.env.OMPW_ALLOW_NON_TTY_PROMPT === "1";
}
...
function printProtectionDetails(protection) {
	...
	if (protection.integrationStatus === "unknown") {
		if (protection.integrationTarget) {
			console.log(`ompw could not verify whether commits are merged into ${protection.integrationDisplay}.`);
		} else {
			console.log("ompw could not verify whether commits are merged because the worktree integration target metadata is missing or incomplete.");
		}
	}
}
...
function getNonInteractiveKeepMessage(session, protection) {
	if (protection.kind === "clean") {
		return `ompw: keeping clean worktree '${session.name}' because no interactive cleanup prompt is available.`;
	}

	if (protection.kind === "dirty") {
		return `ompw: keeping dirty worktree '${session.name}' (no interactive prompt available).`;
	}

	if (protection.kind === "unintegrated") {
		return `ompw: keeping worktree '${session.name}' because it has commits not merged into ${protection.integrationDisplay} (no interactive prompt available).`;
	}

	return `ompw: keeping protected worktree '${session.name}' (no interactive prompt available).`;
}
...
export async function promptProtectedAction(session, protection) {
	...
	console.log("");
	console.log(`This omp session was running in worktree '${session.name}'.`);
