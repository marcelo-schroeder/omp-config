	...
	for (let index = 0; index < args.length; index += 1) {
		...
		switch (arg) {
			...
			case "--debug":
				...
				break;
			case "--omp-bin":
				options.ompBin = takeOptionValue(args, index, arg);
				index += 1;
				break;
			default:
				if (arg.startsWith("--")) {
...
export function parseArgs(argv) {
	...
	options.ompArgs = separatorIndex === -1 ? [] : argv.slice(separatorIndex + 1);
...
export function getHelpText() {
	return [
		"ompw - git worktree wrapper for omp",
		"",
		"Usage:",
		...
		"  --omp-bin <path>    Override the omp executable (or use OMPW_OMP_BIN)",
		...
		"  - When --base is omitted, ompw may create from origin/<target> instead of a diverged local branch.",
		"  - Managed branches use the prefix 'ompw/'.",
		"  - Managed worktrees are stored beside the repo in '<repo>.worktrees/<name>'.",
		"  - Hooks are configured via ompw.config.json and/or .ompw.local.json.",
		"  - Extra omp arguments must come after '--'.",
	].join("\n");
}
