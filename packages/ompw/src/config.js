...
export const OMPW_SHARED_CONFIG_FILENAME = "ompw.config.json";
export const OMPW_LOCAL_CONFIG_FILENAME = ".ompw.local.json";
...
function createConfigError(message, filePath) {
	return new Error(`Invalid ompw hook config at '${filePath}': ${message}`);
}
...
async function readJsonConfigFile(filePath) {
	try {
		...
		try {
			...
			throw new Error(`Failed to parse ompw hook config at '${filePath}'.`);
		}
	} catch (error) {
...
export async function loadOmpwHookConfig({ sharedConfigRoot, localConfigRoot }) {
	const sharedPath = path.join(sharedConfigRoot, OMPW_SHARED_CONFIG_FILENAME);
	const localPath = path.join(localConfigRoot, OMPW_LOCAL_CONFIG_FILENAME);
