...
test("creates a named managed worktree, persists metadata, and launches omp with worktree awareness", async () => {
	...
	try {
		...
		assert.equal(capture.env.PI_WORKTREE_BRANCH, "ompw/feature-auth");
		...
		assert.ok(
			capture.argv.some((arg) => arg.endsWith("packages/ompw/extensions/worktree-awareness/index.ts")),
		...
		assert.equal(metadata.kind, "ompw");
		assert.equal(metadata.name, "feature-auth");
		assert.equal(metadata.branch, "ompw/feature-auth");
		...
		assert.equal(worktreeInfo.kind, "ompw");
...
test("creates an auto-named managed worktree, marks it disposable, and deletes it on clean exit", async () => {
	...
	try {
		...
		assert.equal(capture.env.PI_WORKTREE_BRANCH, `ompw/${name}`);
		assert.equal(capture.env.PI_WORKTREE_PATH, worktreePath);
