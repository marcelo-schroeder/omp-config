...
test("rename updates the managed branch, path, and metadata, and the renamed worktree can be reused", async () => {
	...
	try {
		...
		const renameResult = await runOmpw({
			cwd: repo.repoPath,
		...
		await assertBranchExists(repo.repoPath, "ompw/feature-login");

		const metadata = await readManagedWorktreeMetadata(newPath);
		assert.ok(metadata);
		assert.equal(metadata.name, "feature-login");
		assert.equal(metadata.branch, "ompw/feature-login");
		...
		const pathResult = await runOmpw({
			cwd: repo.repoPath,
		...
		const reuseResult = await runOmpw({
			cwd: repo.repoPath,
			args: ["feature-login"],
			env: { OMPW_FAKE_OMP_CAPTURE: capturePath },
		});
		...
		assert.equal(capture.env.PI_WORKTREE_BRANCH, "ompw/feature-login");
		assert.ok(worktreeInfo);
		assert.equal(worktreeInfo.name, "feature-login");
		assert.equal(worktreeInfo.branch, "ompw/feature-login");
...
test("rename fails when the target managed worktree name already exists", async () => {
	...
	try {
		const createAuth = await runOmpw({
			cwd: repo.repoPath,
			args: ["feature-auth"],
		});
		assert.equal(createAuth.code, 0);

		const createLogin = await runOmpw({
			cwd: repo.repoPath,
			args: ["feature-login"],
		});
...
test("rename refuses to rename the active worktree from inside itself", async () => {
	...
	try {
		...
		assert.ok(!worktreePaths.includes(newPath));
		await assertBranchExists(repo.repoPath, "ompw/feature-auth");
		await assertBranchMissing(repo.repoPath, "ompw/feature-login");
	} finally {
		await repo.cleanup();
	}
});
...
test("rename preserves metadata-incomplete managed worktrees as metadata-incomplete", async () => {
	...
	try {
		await git(["worktree", "add", "-b", "ompw/legacy", oldPath, "main"], repo.repoPath);
		...
		assert.ok(!worktreePaths.includes(oldPath));
		assert.ok(worktreePaths.includes(newPath));
		await assertBranchMissing(repo.repoPath, "ompw/legacy");
		await assertBranchExists(repo.repoPath, "ompw/legacy-renamed");
		...
		assert.equal(worktreeInfo.branch, "ompw/legacy-renamed");
