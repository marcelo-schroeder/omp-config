...
async function main() {
	const capturePath = process.env.OMPW_FAKE_OMP_CAPTURE;
	if (capturePath) {
	...
	const touchPath = process.env.OMPW_FAKE_OMP_TOUCH;
	if (touchPath) {
	...
	const commitPath = process.env.OMPW_FAKE_OMP_COMMIT;
	if (commitPath) {
		...
		await runGit(["commit", "-m", process.env.OMPW_FAKE_OMP_COMMIT_MESSAGE || `test commit ${path.basename(relativeCommitPath)}`]);
	}

	const exitCode = Number.parseInt(process.env.OMPW_FAKE_OMP_EXIT_CODE || "0", 10);
