...
async function main() {
	const capturePath = process.env.OMPW_HOOK_CAPTURE_PATH;
	if (!capturePath) {
		throw new Error("OMPW_HOOK_CAPTURE_PATH is required.");
	}

	const records = await readExistingRecords(capturePath);
	const capturedEnv = Object.fromEntries(
		Object.entries(process.env).filter(([key]) => key.startsWith("PI_WORKTREE_") || key.startsWith("OMPW_HOOK_")),
	);
	...
	const exitCode = Number.parseInt(process.env.OMPW_TEST_HOOK_EXIT_CODE || "0", 10);
