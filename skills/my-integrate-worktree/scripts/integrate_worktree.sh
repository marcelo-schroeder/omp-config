...
#
# Fast-forwards a recorded target branch with a rebased ompw worktree branch and
...
usage() {
  cat <<USAGE
Fast-forward a recorded target branch with an ompw worktree branch.
...
Arguments:
  worktree-name          Optional convenience form; derives branch as ompw/<worktree-name>
...
Examples:
  $(basename "$0") --repo-root /repo --branch ompw/eager-mesa-heron --target-remote origin --target-branch main
  $(basename "$0") --repo-root /repo eager-mesa-heron --target-remote origin --target-branch main --dry-run
  $(basename "$0") --branch ompw/my-feature --target-remote upstream --target-branch develop --skip-push
USAGE
}

resolve_repo_root() {
  if [[ -n "$EXPLICIT_REPO_ROOT" ]]; then
    git -C "$EXPLICIT_REPO_ROOT" rev-parse --show-toplevel >/dev/null || {
